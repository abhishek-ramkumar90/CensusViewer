package com.CensusViewer.census.model;

import java.math.BigDecimal;
import java.util.Collection;

import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.Id;
import javax.persistence.Table;


@Entity(name="towns")
@Table(name="towns")
public class Town {
	@Id
	@Column(name = "gid")
	private BigDecimal gid ;
	
	private String  statecode;
	private String  tehsilcode;
	private String  level ;
	private BigDecimal  no_hh ;
	private BigDecimal   tot_p ;
	private BigDecimal   tot_m ;
	private BigDecimal   tot_f ;
	private BigDecimal   p_06 ;
	private BigDecimal   m_06 ;
	private BigDecimal   f_06 ;
	private BigDecimal  p_sc ;
	private BigDecimal  m_sc ;
	private BigDecimal   f_sc ;
	private BigDecimal   p_st ;
	private BigDecimal   m_st ;
	private BigDecimal   f_st ;
	private BigDecimal   p_lit ;
	private BigDecimal   m_lit ;
	private BigDecimal   f_lit ;
	private BigDecimal   p_ill ;
	private BigDecimal   m_ill ;
	private BigDecimal   f_ill ;
	private BigDecimal   tot_work_p ;
	private BigDecimal 	  tot_work_m ;
	private BigDecimal   tot_work_f ;
	private BigDecimal   mainwork_p ;
	private BigDecimal   mainwork_m ;
	private BigDecimal   mainwork_f ;
	private BigDecimal  main_cl_p ;
	private BigDecimal   main_cl_m ;
	private BigDecimal   main_cl_f ;
	private BigDecimal   main_al_p ;
	private BigDecimal   main_al_m ;
	private BigDecimal   main_al_f ;
	private BigDecimal   main_hh_p ;
	private BigDecimal   main_hh_m ;
	private BigDecimal   main_hh_f ;
	private BigDecimal   main_ot_p ;
	private BigDecimal   main_ot_m ;
	private BigDecimal   main_ot_f ;
	private BigDecimal   margwork_p ;
	private BigDecimal   margwork_m ;
	private BigDecimal   margwork_f ;
	private BigDecimal   marg_cl_p ;
	private BigDecimal   marg_cl_m ;
	private BigDecimal   marg_cl_f ;
	private BigDecimal   marg_al_p ;
	private BigDecimal   marg_al_m ;
	private BigDecimal   marg_al_f ;
	private BigDecimal   marg_hh_p ;
	private BigDecimal 	  marg_hh_m ;
	private BigDecimal   marg_hh_f ;
	private BigDecimal   marg_ot_p ;
	private BigDecimal   marg_ot_m ;
	private BigDecimal   marg_ot_f ;
	private BigDecimal   non_work_p ;
	private BigDecimal   non_work_m ;
	private BigDecimal   non_work_f ;
	private Integer   sexratio ;
	private Integer   sex_rt_sc ;
	private Integer  sex_rt_st;
	private BigDecimal   hh_size ;
	private BigDecimal   sc_per ;
	private BigDecimal   st_per ;
	private BigDecimal   m_lit_rate ;
	private BigDecimal   f_lit_rate ;
	private BigDecimal  gender_gap ;
	private BigDecimal  p_lit_per ;
	private BigDecimal  m_lit_per ;
	private BigDecimal  f_lit_per ;
	private BigDecimal  tot_p_per ;
	private BigDecimal  t_wor_pr_p ;
	private BigDecimal  t_wor_pr_m ;
	private BigDecimal  t_wor_pr_f ;
	private BigDecimal  t_mn_pr_p ;
	private BigDecimal  t_mn_pr_m ;
	private BigDecimal  t_mn_pr_f ;
	private BigDecimal  t_mrg_pr_p ;
	private BigDecimal  t_mrg_pr_m ;
	private BigDecimal  t_mrg_pr_f ;
	private BigDecimal  n_wor_pr_p ;
	private BigDecimal  n_wor_pr_m ;
	private BigDecimal	  n_wor_pr_f ;
	private BigDecimal  cl_per_p ;
	private BigDecimal  cl_per_m ;
	private BigDecimal  cl_per_f ;
	private BigDecimal  al_per_p ;
	private BigDecimal  al_per_m ;
	private BigDecimal  al_per_f ;
	private BigDecimal  hh_per_p ;
	private BigDecimal  hh_per_m ;
	private BigDecimal  hh_per_f ;
	private BigDecimal  ow_per_p ;
	private BigDecimal  ow_per_m ;
	private BigDecimal  ow_per_f ;
	private BigDecimal  cl_mm_p ;
	private BigDecimal  cl_mm_m ;
	private BigDecimal  cl_mm_f ;
	private BigDecimal  al_mm_p ;
	private BigDecimal  al_mm_m ;
	private BigDecimal  al_mm_f ;
	private BigDecimal  hh_mm_p ;
	private BigDecimal  hh_mm_m ;
	private BigDecimal  hh_mm_f ;
	private BigDecimal  ow_mm_p ;
	private BigDecimal  ow_mm_m ;
	private BigDecimal  ow_mm_f ;
	private BigDecimal  wpr ;
	private String  town_v_cod ;
	private String  state;
	private String  district;
	private String  dist_hq ;
	private String  state_hq ;
	private String  tehsil_hq ;
	private String key;
	private String  town_name ;
	private  BigDecimal   area ;
	private double  household ;
	private double  pop_1901 ;
	private double  pop_1911 ;
	private  BigDecimal   gr_01_11 ;
	private double  pop_1921 ;
	private BigDecimal   gr_11_21 ;
	private double  pop_1931 ;
	private  BigDecimal   gr_21_31 ;
	private double  pop_1941 ;
	private  BigDecimal  gr_31_41 ;
	private double  pop_1951 ;
	private String  gr_41_51 ;
	private double  pop_1961 ;
	private  BigDecimal  gr_51_61 ;
	private double  pop_1971 ;
	private  BigDecimal gr_61_71 ;
	private BigDecimal pop_1981 ;
	private  BigDecimal gr_71_81 ;
	private  BigDecimal pop_1991 ;
	private  BigDecimal gr_81_91 ;
	private  BigDecimal pop_2001 ;
	private  BigDecimal  gr_91_2001 ;
	private  BigDecimal density ;
	private  BigDecimal sex_1981 ;
	private  BigDecimal  sex_1991 ;
	private  BigDecimal sex_2001 ;
	private  BigDecimal avg_rain ;
	private  BigDecimal max_temp ;
	private  BigDecimal min_temp ;
	private String state_h_na ;
	private String sh_dist_km ;
	private String dist_h_na;
	private  BigDecimal dh_dist_km ;
	private String hd_h_na;
	private  BigDecimal sd_dist_km ;
	private String dist_th_hq;
	private  BigDecimal th_dist_hq ;
	private String city_1lakh;
	private  BigDecimal city_dt_km ;
	private  String railway_st;
	private  BigDecimal rl_dist_km ;
	private  String bus_route ;
	private  BigDecimal br_dist_km ;
	private  String nav_r_c ;
	private  String nv_dist_km ;
	private  BigDecimal rec_tax ;
	private  BigDecimal rev_m_t ;
	private  BigDecimal govt_grant ;
	private  BigDecimal loan ;
	private  BigDecimal advance ;
	private  String oth_sour ;
	private  BigDecimal tot_rec ;
	private  BigDecimal gen_adm ;
	private  BigDecimal pub_saf ;
	private  BigDecimal pub_h_c ;
	private  BigDecimal pub_works ;
	private  BigDecimal pub_inst ;
	private  String oth_exp ;
	private  BigDecimal tot_exp ;
	private  BigDecimal k_road ;
	private  BigDecimal p_road ;
	private  String sewer ;
	private  double water_bor ;
	private  double service ;
	private  double lat_oth ;
	private  String meth_d_n_s ;
	private  String sou_of_sup ;
	private  String sys_stor ;
	private  String stor_cap ;
	private  String fire_f_ser ;
	private  String f_f_name ;
	private  BigDecimal dist_f_ser ;
	private  double el_dom ;
	private  double el_ind ;
	private  double el_com ;
	private double el_road_l ;
	private  double el_others ;
	private  double hospital ;
	private  double hos_beds ;
	private  String ht_name ;
	private  double disp ;
	private  double disp_beds ;
	private String dt_name ;
	private  double health_ctr ;
	private  double  hc_beds ;
	private  String hct_name ;
	private  double f_w_ctr ;
	private  double fwc_beds ;
	private  String fwct_name ;
	private  double tb_clinic ;
	private  double tbc_beds ;
	private  String tbc_name ;
	private  double nur_homes ;
	private  double  nh_beds ;
	private  String nht_name ;
	private  double med_oth ;
	private  double othmi_beds ;
	private  double  arts ;
	private  double science ;
	private  double commerce ;
	private  double arts_sci ;
	private  double  arts_comm ;
	private  double  art_sc_co ;
	private  double law ;
	private  double univ ;
	private  double oth_col ;
	private  String  as_near_km ;
	private  double medical ;
	private  String md_near_km ;
	private  double engg ;
	private  String  en_near_km ;
	private  double polytech ;
	private  String poly_near ;
	private  double vt_inst ;
	private  double sh_inst ;
	private  double  type_inst ;
	private  double sh_type_in ;
	private  double oth_inst_s ;
	private BigDecimal s_sec ;
	private  String ss_near ;
	private  double secondary ;
	private String sec_near ;
	private  double  middle ;
	private  String  mid_near ;
	private  double __primary ;
	private  String prim_near ;
	private  double  adult_cnt ;
	private  String alc_near ;
	private  double oth_cntr ;
	private  double n_hostels ;
	private  double n_seats ;
	private String wwh_dist ;
	private  double rc_fac ;
	private  double stadium ;
	private  double cinema ;
	private  double auditorium ;
	private  double p_lib ;
	private  double  r_rooms ;
	private String  imp_com1 ;
	private String  imp_com2 ;
	private String  imp_com3 ;
	private String   exp_comm1 ;
	private String 	  exp_comm2 ;
	private String  exp_comm3 ;
	private String   man_comm1 ;
	private String   man_comm2 ;
	private String   man_comm3 ;
	private  double banks ;
	private  double a_c_soc ;
	private  double n_a_c_soc ;
	private String  distcode ;
	private String  towncode ;
	private Integer stateid ;
	private Integer distid ;
	private Integer tehsilid ;
	
	
	public BigDecimal getGid() {
		return gid;
	}
	public void setGid(BigDecimal gid) {
		this.gid = gid;
	}
	
	@Column(name = "statecode")
	public String getStatecode() {
		return statecode;
	}
	public void setStatecode(String statecode) {
		this.statecode = statecode;
	}
	@Column(name = "tehsilcode")
	public String getTehsilcode() {
		return tehsilcode;
	}
	public void setTehsilcode(String tehsilcode) {
		this.tehsilcode = tehsilcode;
	}
	public String getLevel() {
		return level;
	}
	public void setLevel(String level) {
		this.level = level;
	}
	public BigDecimal getNo_hh() {
		return no_hh;
	}
	public void setNo_hh(BigDecimal no_hh) {
		this.no_hh = no_hh;
	}
	public BigDecimal getTot_p() {
		return tot_p;
	}
	public void setTot_p(BigDecimal tot_p) {
		this.tot_p = tot_p;
	}
	public BigDecimal getTot_m() {
		return tot_m;
	}
	public void setTot_m(BigDecimal tot_m) {
		this.tot_m = tot_m;
	}
	public BigDecimal getTot_f() {
		return tot_f;
	}
	public void setTot_f(BigDecimal tot_f) {
		this.tot_f = tot_f;
	}
	public BigDecimal getP_06() {
		return p_06;
	}
	public void setP_06(BigDecimal p_06) {
		this.p_06 = p_06;
	}
	public BigDecimal getM_06() {
		return m_06;
	}
	public void setM_06(BigDecimal m_06) {
		this.m_06 = m_06;
	}
	public BigDecimal getF_06() {
		return f_06;
	}
	public void setF_06(BigDecimal f_06) {
		this.f_06 = f_06;
	}
	public BigDecimal getP_sc() {
		return p_sc;
	}
	public void setP_sc(BigDecimal p_sc) {
		this.p_sc = p_sc;
	}
	public BigDecimal getM_sc() {
		return m_sc;
	}
	public void setM_sc(BigDecimal m_sc) {
		this.m_sc = m_sc;
	}
	public BigDecimal getF_sc() {
		return f_sc;
	}
	public void setF_sc(BigDecimal f_sc) {
		this.f_sc = f_sc;
	}
	public BigDecimal getP_st() {
		return p_st;
	}
	public void setP_st(BigDecimal p_st) {
		this.p_st = p_st;
	}
	public BigDecimal getM_st() {
		return m_st;
	}
	public void setM_st(BigDecimal m_st) {
		this.m_st = m_st;
	}
	public BigDecimal getF_st() {
		return f_st;
	}
	public void setF_st(BigDecimal f_st) {
		this.f_st = f_st;
	}
	public BigDecimal getP_lit() {
		return p_lit;
	}
	public void setP_lit(BigDecimal p_lit) {
		this.p_lit = p_lit;
	}
	public BigDecimal getM_lit() {
		return m_lit;
	}
	public void setM_lit(BigDecimal m_lit) {
		this.m_lit = m_lit;
	}
	public BigDecimal getF_lit() {
		return f_lit;
	}
	public void setF_lit(BigDecimal f_lit) {
		this.f_lit = f_lit;
	}
	public BigDecimal getP_ill() {
		return p_ill;
	}
	public void setP_ill(BigDecimal p_ill) {
		this.p_ill = p_ill;
	}
	public BigDecimal getM_ill() {
		return m_ill;
	}
	public void setM_ill(BigDecimal m_ill) {
		this.m_ill = m_ill;
	}
	public BigDecimal getF_ill() {
		return f_ill;
	}
	public void setF_ill(BigDecimal f_ill) {
		this.f_ill = f_ill;
	}
	public BigDecimal getTot_work_p() {
		return tot_work_p;
	}
	public void setTot_work_p(BigDecimal tot_work_p) {
		this.tot_work_p = tot_work_p;
	}
	public BigDecimal getTot_work_m() {
		return tot_work_m;
	}
	public void setTot_work_m(BigDecimal tot_work_m) {
		this.tot_work_m = tot_work_m;
	}
	public BigDecimal getTot_work_f() {
		return tot_work_f;
	}
	public void setTot_work_f(BigDecimal tot_work_f) {
		this.tot_work_f = tot_work_f;
	}
	public BigDecimal getMainwork_p() {
		return mainwork_p;
	}
	public void setMainwork_p(BigDecimal mainwork_p) {
		this.mainwork_p = mainwork_p;
	}
	public BigDecimal getMainwork_m() {
		return mainwork_m;
	}
	public void setMainwork_m(BigDecimal mainwork_m) {
		this.mainwork_m = mainwork_m;
	}
	public BigDecimal getMainwork_f() {
		return mainwork_f;
	}
	public void setMainwork_f(BigDecimal mainwork_f) {
		this.mainwork_f = mainwork_f;
	}
	public BigDecimal getMain_cl_p() {
		return main_cl_p;
	}
	public void setMain_cl_p(BigDecimal main_cl_p) {
		this.main_cl_p = main_cl_p;
	}
	public BigDecimal getMain_cl_m() {
		return main_cl_m;
	}
	public void setMain_cl_m(BigDecimal main_cl_m) {
		this.main_cl_m = main_cl_m;
	}
	public BigDecimal getMain_cl_f() {
		return main_cl_f;
	}
	public void setMain_cl_f(BigDecimal main_cl_f) {
		this.main_cl_f = main_cl_f;
	}
	public BigDecimal getMain_al_p() {
		return main_al_p;
	}
	public void setMain_al_p(BigDecimal main_al_p) {
		this.main_al_p = main_al_p;
	}
	public BigDecimal getMain_al_m() {
		return main_al_m;
	}
	public void setMain_al_m(BigDecimal main_al_m) {
		this.main_al_m = main_al_m;
	}
	public BigDecimal getMain_al_f() {
		return main_al_f;
	}
	public void setMain_al_f(BigDecimal main_al_f) {
		this.main_al_f = main_al_f;
	}
	public BigDecimal getMain_hh_p() {
		return main_hh_p;
	}
	public void setMain_hh_p(BigDecimal main_hh_p) {
		this.main_hh_p = main_hh_p;
	}
	public BigDecimal getMain_hh_m() {
		return main_hh_m;
	}
	public void setMain_hh_m(BigDecimal main_hh_m) {
		this.main_hh_m = main_hh_m;
	}
	public BigDecimal getMain_hh_f() {
		return main_hh_f;
	}
	public void setMain_hh_f(BigDecimal main_hh_f) {
		this.main_hh_f = main_hh_f;
	}
	public BigDecimal getMain_ot_p() {
		return main_ot_p;
	}
	public void setMain_ot_p(BigDecimal main_ot_p) {
		this.main_ot_p = main_ot_p;
	}
	public BigDecimal getMain_ot_m() {
		return main_ot_m;
	}
	public void setMain_ot_m(BigDecimal main_ot_m) {
		this.main_ot_m = main_ot_m;
	}
	public BigDecimal getMain_ot_f() {
		return main_ot_f;
	}
	public void setMain_ot_f(BigDecimal main_ot_f) {
		this.main_ot_f = main_ot_f;
	}
	public BigDecimal getMargwork_p() {
		return margwork_p;
	}
	public void setMargwork_p(BigDecimal margwork_p) {
		this.margwork_p = margwork_p;
	}
	public BigDecimal getMargwork_m() {
		return margwork_m;
	}
	public void setMargwork_m(BigDecimal margwork_m) {
		this.margwork_m = margwork_m;
	}
	public BigDecimal getMargwork_f() {
		return margwork_f;
	}
	public void setMargwork_f(BigDecimal margwork_f) {
		this.margwork_f = margwork_f;
	}
	public BigDecimal getMarg_cl_p() {
		return marg_cl_p;
	}
	public void setMarg_cl_p(BigDecimal marg_cl_p) {
		this.marg_cl_p = marg_cl_p;
	}
	public BigDecimal getMarg_cl_m() {
		return marg_cl_m;
	}
	public void setMarg_cl_m(BigDecimal marg_cl_m) {
		this.marg_cl_m = marg_cl_m;
	}
	public BigDecimal getMarg_cl_f() {
		return marg_cl_f;
	}
	public void setMarg_cl_f(BigDecimal marg_cl_f) {
		this.marg_cl_f = marg_cl_f;
	}
	public BigDecimal getMarg_al_p() {
		return marg_al_p;
	}
	public void setMarg_al_p(BigDecimal marg_al_p) {
		this.marg_al_p = marg_al_p;
	}
	public BigDecimal getMarg_al_m() {
		return marg_al_m;
	}
	public void setMarg_al_m(BigDecimal marg_al_m) {
		this.marg_al_m = marg_al_m;
	}
	public BigDecimal getMarg_al_f() {
		return marg_al_f;
	}
	public void setMarg_al_f(BigDecimal marg_al_f) {
		this.marg_al_f = marg_al_f;
	}
	public BigDecimal getMarg_hh_p() {
		return marg_hh_p;
	}
	public void setMarg_hh_p(BigDecimal marg_hh_p) {
		this.marg_hh_p = marg_hh_p;
	}
	public BigDecimal getMarg_hh_m() {
		return marg_hh_m;
	}
	public void setMarg_hh_m(BigDecimal marg_hh_m) {
		this.marg_hh_m = marg_hh_m;
	}
	public BigDecimal getMarg_hh_f() {
		return marg_hh_f;
	}
	public void setMarg_hh_f(BigDecimal marg_hh_f) {
		this.marg_hh_f = marg_hh_f;
	}
	public BigDecimal getMarg_ot_p() {
		return marg_ot_p;
	}
	public void setMarg_ot_p(BigDecimal marg_ot_p) {
		this.marg_ot_p = marg_ot_p;
	}
	public BigDecimal getMarg_ot_m() {
		return marg_ot_m;
	}
	public void setMarg_ot_m(BigDecimal marg_ot_m) {
		this.marg_ot_m = marg_ot_m;
	}
	public BigDecimal getMarg_ot_f() {
		return marg_ot_f;
	}
	public void setMarg_ot_f(BigDecimal marg_ot_f) {
		this.marg_ot_f = marg_ot_f;
	}
	public BigDecimal getNon_work_p() {
		return non_work_p;
	}
	public void setNon_work_p(BigDecimal non_work_p) {
		this.non_work_p = non_work_p;
	}
	public BigDecimal getNon_work_m() {
		return non_work_m;
	}
	public void setNon_work_m(BigDecimal non_work_m) {
		this.non_work_m = non_work_m;
	}
	public BigDecimal getNon_work_f() {
		return non_work_f;
	}
	public void setNon_work_f(BigDecimal non_work_f) {
		this.non_work_f = non_work_f;
	}
	public Integer getSexratio() {
		return sexratio;
	}
	public void setSexratio(Integer sexratio) {
		this.sexratio = sexratio;
	}
	public Integer getSex_rt_sc() {
		return sex_rt_sc;
	}
	public void setSex_rt_sc(Integer sex_rt_sc) {
		this.sex_rt_sc = sex_rt_sc;
	}
	public Integer getSex_rt_st() {
		return sex_rt_st;
	}
	public void setSex_rt_st(Integer sex_rt_st) {
		this.sex_rt_st = sex_rt_st;
	}
	public BigDecimal getHh_size() {
		return hh_size;
	}
	public void setHh_size(BigDecimal hh_size) {
		this.hh_size = hh_size;
	}
	public BigDecimal getSc_per() {
		return sc_per;
	}
	public void setSc_per(BigDecimal sc_per) {
		this.sc_per = sc_per;
	}
	public BigDecimal getSt_per() {
		return st_per;
	}
	public void setSt_per(BigDecimal st_per) {
		this.st_per = st_per;
	}
	public BigDecimal getM_lit_rate() {
		return m_lit_rate;
	}
	public void setM_lit_rate(BigDecimal m_lit_rate) {
		this.m_lit_rate = m_lit_rate;
	}
	public BigDecimal getF_lit_rate() {
		return f_lit_rate;
	}
	public void setF_lit_rate(BigDecimal f_lit_rate) {
		this.f_lit_rate = f_lit_rate;
	}
	public BigDecimal getGender_gap() {
		return gender_gap;
	}
	public void setGender_gap(BigDecimal gender_gap) {
		this.gender_gap = gender_gap;
	}
	public BigDecimal getP_lit_per() {
		return p_lit_per;
	}
	public void setP_lit_per(BigDecimal p_lit_per) {
		this.p_lit_per = p_lit_per;
	}
	public BigDecimal getM_lit_per() {
		return m_lit_per;
	}
	public void setM_lit_per(BigDecimal m_lit_per) {
		this.m_lit_per = m_lit_per;
	}
	public BigDecimal getF_lit_per() {
		return f_lit_per;
	}
	public void setF_lit_per(BigDecimal f_lit_per) {
		this.f_lit_per = f_lit_per;
	}
	public BigDecimal getTot_p_per() {
		return tot_p_per;
	}
	public void setTot_p_per(BigDecimal tot_p_per) {
		this.tot_p_per = tot_p_per;
	}
	public BigDecimal getT_wor_pr_p() {
		return t_wor_pr_p;
	}
	public void setT_wor_pr_p(BigDecimal t_wor_pr_p) {
		this.t_wor_pr_p = t_wor_pr_p;
	}
	public BigDecimal getT_wor_pr_m() {
		return t_wor_pr_m;
	}
	public void setT_wor_pr_m(BigDecimal t_wor_pr_m) {
		this.t_wor_pr_m = t_wor_pr_m;
	}
	public BigDecimal getT_wor_pr_f() {
		return t_wor_pr_f;
	}
	public void setT_wor_pr_f(BigDecimal t_wor_pr_f) {
		this.t_wor_pr_f = t_wor_pr_f;
	}
	public BigDecimal getT_mn_pr_p() {
		return t_mn_pr_p;
	}
	public void setT_mn_pr_p(BigDecimal t_mn_pr_p) {
		this.t_mn_pr_p = t_mn_pr_p;
	}
	public BigDecimal getT_mn_pr_m() {
		return t_mn_pr_m;
	}
	public void setT_mn_pr_m(BigDecimal t_mn_pr_m) {
		this.t_mn_pr_m = t_mn_pr_m;
	}
	public BigDecimal getT_mn_pr_f() {
		return t_mn_pr_f;
	}
	public void setT_mn_pr_f(BigDecimal t_mn_pr_f) {
		this.t_mn_pr_f = t_mn_pr_f;
	}
	public BigDecimal getT_mrg_pr_p() {
		return t_mrg_pr_p;
	}
	public void setT_mrg_pr_p(BigDecimal t_mrg_pr_p) {
		this.t_mrg_pr_p = t_mrg_pr_p;
	}
	public BigDecimal getT_mrg_pr_m() {
		return t_mrg_pr_m;
	}
	public void setT_mrg_pr_m(BigDecimal t_mrg_pr_m) {
		this.t_mrg_pr_m = t_mrg_pr_m;
	}
	public BigDecimal getT_mrg_pr_f() {
		return t_mrg_pr_f;
	}
	public void setT_mrg_pr_f(BigDecimal t_mrg_pr_f) {
		this.t_mrg_pr_f = t_mrg_pr_f;
	}
	public BigDecimal getN_wor_pr_p() {
		return n_wor_pr_p;
	}
	public void setN_wor_pr_p(BigDecimal n_wor_pr_p) {
		this.n_wor_pr_p = n_wor_pr_p;
	}
	public BigDecimal getN_wor_pr_m() {
		return n_wor_pr_m;
	}
	public void setN_wor_pr_m(BigDecimal n_wor_pr_m) {
		this.n_wor_pr_m = n_wor_pr_m;
	}
	public BigDecimal getN_wor_pr_f() {
		return n_wor_pr_f;
	}
	public void setN_wor_pr_f(BigDecimal n_wor_pr_f) {
		this.n_wor_pr_f = n_wor_pr_f;
	}
	public BigDecimal getCl_per_p() {
		return cl_per_p;
	}
	public void setCl_per_p(BigDecimal cl_per_p) {
		this.cl_per_p = cl_per_p;
	}
	public BigDecimal getCl_per_m() {
		return cl_per_m;
	}
	public void setCl_per_m(BigDecimal cl_per_m) {
		this.cl_per_m = cl_per_m;
	}
	public BigDecimal getCl_per_f() {
		return cl_per_f;
	}
	public void setCl_per_f(BigDecimal cl_per_f) {
		this.cl_per_f = cl_per_f;
	}
	public BigDecimal getAl_per_p() {
		return al_per_p;
	}
	public void setAl_per_p(BigDecimal al_per_p) {
		this.al_per_p = al_per_p;
	}
	public BigDecimal getAl_per_m() {
		return al_per_m;
	}
	public void setAl_per_m(BigDecimal al_per_m) {
		this.al_per_m = al_per_m;
	}
	public BigDecimal getAl_per_f() {
		return al_per_f;
	}
	public void setAl_per_f(BigDecimal al_per_f) {
		this.al_per_f = al_per_f;
	}
	public BigDecimal getHh_per_p() {
		return hh_per_p;
	}
	public void setHh_per_p(BigDecimal hh_per_p) {
		this.hh_per_p = hh_per_p;
	}
	public BigDecimal getHh_per_m() {
		return hh_per_m;
	}
	public void setHh_per_m(BigDecimal hh_per_m) {
		this.hh_per_m = hh_per_m;
	}
	public BigDecimal getHh_per_f() {
		return hh_per_f;
	}
	public void setHh_per_f(BigDecimal hh_per_f) {
		this.hh_per_f = hh_per_f;
	}
	public BigDecimal getOw_per_p() {
		return ow_per_p;
	}
	public void setOw_per_p(BigDecimal ow_per_p) {
		this.ow_per_p = ow_per_p;
	}
	public BigDecimal getOw_per_m() {
		return ow_per_m;
	}
	public void setOw_per_m(BigDecimal ow_per_m) {
		this.ow_per_m = ow_per_m;
	}
	public BigDecimal getOw_per_f() {
		return ow_per_f;
	}
	public void setOw_per_f(BigDecimal ow_per_f) {
		this.ow_per_f = ow_per_f;
	}
	public BigDecimal getCl_mm_p() {
		return cl_mm_p;
	}
	public void setCl_mm_p(BigDecimal cl_mm_p) {
		this.cl_mm_p = cl_mm_p;
	}
	public BigDecimal getCl_mm_m() {
		return cl_mm_m;
	}
	public void setCl_mm_m(BigDecimal cl_mm_m) {
		this.cl_mm_m = cl_mm_m;
	}
	public BigDecimal getCl_mm_f() {
		return cl_mm_f;
	}
	public void setCl_mm_f(BigDecimal cl_mm_f) {
		this.cl_mm_f = cl_mm_f;
	}
	public BigDecimal getAl_mm_p() {
		return al_mm_p;
	}
	public void setAl_mm_p(BigDecimal al_mm_p) {
		this.al_mm_p = al_mm_p;
	}
	public BigDecimal getAl_mm_m() {
		return al_mm_m;
	}
	public void setAl_mm_m(BigDecimal al_mm_m) {
		this.al_mm_m = al_mm_m;
	}
	public BigDecimal getAl_mm_f() {
		return al_mm_f;
	}
	public void setAl_mm_f(BigDecimal al_mm_f) {
		this.al_mm_f = al_mm_f;
	}
	public BigDecimal getHh_mm_p() {
		return hh_mm_p;
	}
	public void setHh_mm_p(BigDecimal hh_mm_p) {
		this.hh_mm_p = hh_mm_p;
	}
	public BigDecimal getHh_mm_m() {
		return hh_mm_m;
	}
	public void setHh_mm_m(BigDecimal hh_mm_m) {
		this.hh_mm_m = hh_mm_m;
	}
	public BigDecimal getHh_mm_f() {
		return hh_mm_f;
	}
	public void setHh_mm_f(BigDecimal hh_mm_f) {
		this.hh_mm_f = hh_mm_f;
	}
	public BigDecimal getOw_mm_p() {
		return ow_mm_p;
	}
	public void setOw_mm_p(BigDecimal ow_mm_p) {
		this.ow_mm_p = ow_mm_p;
	}
	public BigDecimal getOw_mm_m() {
		return ow_mm_m;
	}
	public void setOw_mm_m(BigDecimal ow_mm_m) {
		this.ow_mm_m = ow_mm_m;
	}
	public BigDecimal getOw_mm_f() {
		return ow_mm_f;
	}
	public void setOw_mm_f(BigDecimal ow_mm_f) {
		this.ow_mm_f = ow_mm_f;
	}
	public BigDecimal getWpr() {
		return wpr;
	}
	public void setWpr(BigDecimal wpr) {
		this.wpr = wpr;
	}
	public String getTown_v_cod() {
		return town_v_cod;
	}
	public void setTown_v_cod(String town_v_cod) {
		this.town_v_cod = town_v_cod;
	}
	public String getState() {
		return state;
	}
	public void setState(String state) {
		this.state = state;
	}
	public String getDistrict() {
		return district;
	}
	public void setDistrict(String district) {
		this.district = district;
	}
	public String getDist_hq() {
		return dist_hq;
	}
	public void setDist_hq(String dist_hq) {
		this.dist_hq = dist_hq;
	}
	public String getState_hq() {
		return state_hq;
	}
	public void setState_hq(String state_hq) {
		this.state_hq = state_hq;
	}
	public String getTehsil_hq() {
		return tehsil_hq;
	}
	public void setTehsil_hq(String tehsil_hq) {
		this.tehsil_hq = tehsil_hq;
	}
	public String getKey() {
		return key;
	}
	public void setKey(String key) {
		this.key = key;
	}
	
	@Column(name = "town_name")
	public String getTown_name() {
		return town_name;
	}
	public void setTown_name(String town_name) {
		this.town_name = town_name;
	}
	public BigDecimal getArea() {
		return area;
	}
	public void setArea(BigDecimal area) {
		this.area = area;
	}
	public double getHousehold() {
		return household;
	}
	public void setHousehold(double household) {
		this.household = household;
	}
	public double getPop_1901() {
		return pop_1901;
	}
	public void setPop_1901(double pop_1901) {
		this.pop_1901 = pop_1901;
	}
	public double getPop_1911() {
		return pop_1911;
	}
	public void setPop_1911(double pop_1911) {
		this.pop_1911 = pop_1911;
	}
	public BigDecimal getGr_01_11() {
		return gr_01_11;
	}
	public void setGr_01_11(BigDecimal gr_01_11) {
		this.gr_01_11 = gr_01_11;
	}
	public double getPop_1921() {
		return pop_1921;
	}
	public void setPop_1921(double pop_1921) {
		this.pop_1921 = pop_1921;
	}
	public BigDecimal getGr_11_21() {
		return gr_11_21;
	}
	public void setGr_11_21(BigDecimal gr_11_21) {
		this.gr_11_21 = gr_11_21;
	}
	public double getPop_1931() {
		return pop_1931;
	}
	public void setPop_1931(double pop_1931) {
		this.pop_1931 = pop_1931;
	}
	public BigDecimal getGr_21_31() {
		return gr_21_31;
	}
	public void setGr_21_31(BigDecimal gr_21_31) {
		this.gr_21_31 = gr_21_31;
	}
	public double getPop_1941() {
		return pop_1941;
	}
	public void setPop_1941(double pop_1941) {
		this.pop_1941 = pop_1941;
	}
	public BigDecimal getGr_31_41() {
		return gr_31_41;
	}
	public void setGr_31_41(BigDecimal gr_31_41) {
		this.gr_31_41 = gr_31_41;
	}
	public double getPop_1951() {
		return pop_1951;
	}
	public void setPop_1951(double pop_1951) {
		this.pop_1951 = pop_1951;
	}
	public String getGr_41_51() {
		return gr_41_51;
	}
	public void setGr_41_51(String gr_41_51) {
		this.gr_41_51 = gr_41_51;
	}
	public double getPop_1961() {
		return pop_1961;
	}
	public void setPop_1961(double pop_1961) {
		this.pop_1961 = pop_1961;
	}
	public BigDecimal getGr_51_61() {
		return gr_51_61;
	}
	public void setGr_51_61(BigDecimal gr_51_61) {
		this.gr_51_61 = gr_51_61;
	}
	public double getPop_1971() {
		return pop_1971;
	}
	public void setPop_1971(double pop_1971) {
		this.pop_1971 = pop_1971;
	}
	public BigDecimal getGr_61_71() {
		return gr_61_71;
	}
	public void setGr_61_71(BigDecimal gr_61_71) {
		this.gr_61_71 = gr_61_71;
	}
	public BigDecimal getPop_1981() {
		return pop_1981;
	}
	public void setPop_1981(BigDecimal pop_1981) {
		this.pop_1981 = pop_1981;
	}
	public BigDecimal getGr_71_81() {
		return gr_71_81;
	}
	public void setGr_71_81(BigDecimal gr_71_81) {
		this.gr_71_81 = gr_71_81;
	}
	public BigDecimal getPop_1991() {
		return pop_1991;
	}
	public void setPop_1991(BigDecimal pop_1991) {
		this.pop_1991 = pop_1991;
	}
	public BigDecimal getGr_81_91() {
		return gr_81_91;
	}
	public void setGr_81_91(BigDecimal gr_81_91) {
		this.gr_81_91 = gr_81_91;
	}
	public BigDecimal getPop_2001() {
		return pop_2001;
	}
	public void setPop_2001(BigDecimal pop_2001) {
		this.pop_2001 = pop_2001;
	}
	public BigDecimal getGr_91_2001() {
		return gr_91_2001;
	}
	public void setGr_91_2001(BigDecimal gr_91_2001) {
		this.gr_91_2001 = gr_91_2001;
	}
	public BigDecimal getDensity() {
		return density;
	}
	public void setDensity(BigDecimal density) {
		this.density = density;
	}
	public BigDecimal getSex_1981() {
		return sex_1981;
	}
	public void setSex_1981(BigDecimal sex_1981) {
		this.sex_1981 = sex_1981;
	}
	public BigDecimal getSex_1991() {
		return sex_1991;
	}
	public void setSex_1991(BigDecimal sex_1991) {
		this.sex_1991 = sex_1991;
	}
	public BigDecimal getSex_2001() {
		return sex_2001;
	}
	public void setSex_2001(BigDecimal sex_2001) {
		this.sex_2001 = sex_2001;
	}
	public BigDecimal getAvg_rain() {
		return avg_rain;
	}
	public void setAvg_rain(BigDecimal avg_rain) {
		this.avg_rain = avg_rain;
	}
	public BigDecimal getMax_temp() {
		return max_temp;
	}
	public void setMax_temp(BigDecimal max_temp) {
		this.max_temp = max_temp;
	}
	public BigDecimal getMin_temp() {
		return min_temp;
	}
	public void setMin_temp(BigDecimal min_temp) {
		this.min_temp = min_temp;
	}
	public String getState_h_na() {
		return state_h_na;
	}
	public void setState_h_na(String state_h_na) {
		this.state_h_na = state_h_na;
	}
	public String getSh_dist_km() {
		return sh_dist_km;
	}
	public void setSh_dist_km(String sh_dist_km) {
		this.sh_dist_km = sh_dist_km;
	}
	public String getDist_h_na() {
		return dist_h_na;
	}
	public void setDist_h_na(String dist_h_na) {
		this.dist_h_na = dist_h_na;
	}
	public BigDecimal getDh_dist_km() {
		return dh_dist_km;
	}
	public void setDh_dist_km(BigDecimal dh_dist_km) {
		this.dh_dist_km = dh_dist_km;
	}
	public String getHd_h_na() {
		return hd_h_na;
	}
	public void setHd_h_na(String hd_h_na) {
		this.hd_h_na = hd_h_na;
	}
	public BigDecimal getSd_dist_km() {
		return sd_dist_km;
	}
	public void setSd_dist_km(BigDecimal sd_dist_km) {
		this.sd_dist_km = sd_dist_km;
	}
	public String getDist_th_hq() {
		return dist_th_hq;
	}
	public void setDist_th_hq(String dist_th_hq) {
		this.dist_th_hq = dist_th_hq;
	}
	public BigDecimal getTh_dist_hq() {
		return th_dist_hq;
	}
	public void setTh_dist_hq(BigDecimal th_dist_hq) {
		this.th_dist_hq = th_dist_hq;
	}
	public String getCity_1lakh() {
		return city_1lakh;
	}
	public void setCity_1lakh(String city_1lakh) {
		this.city_1lakh = city_1lakh;
	}
	public BigDecimal getCity_dt_km() {
		return city_dt_km;
	}
	public void setCity_dt_km(BigDecimal city_dt_km) {
		this.city_dt_km = city_dt_km;
	}
	public String getRailway_st() {
		return railway_st;
	}
	public void setRailway_st(String railway_st) {
		this.railway_st = railway_st;
	}
	public BigDecimal getRl_dist_km() {
		return rl_dist_km;
	}
	public void setRl_dist_km(BigDecimal rl_dist_km) {
		this.rl_dist_km = rl_dist_km;
	}
	public String getBus_route() {
		return bus_route;
	}
	public void setBus_route(String bus_route) {
		this.bus_route = bus_route;
	}
	public BigDecimal getBr_dist_km() {
		return br_dist_km;
	}
	public void setBr_dist_km(BigDecimal br_dist_km) {
		this.br_dist_km = br_dist_km;
	}
	public String getNav_r_c() {
		return nav_r_c;
	}
	public void setNav_r_c(String nav_r_c) {
		this.nav_r_c = nav_r_c;
	}
	public String getNv_dist_km() {
		return nv_dist_km;
	}
	public void setNv_dist_km(String nv_dist_km) {
		this.nv_dist_km = nv_dist_km;
	}
	public BigDecimal getRec_tax() {
		return rec_tax;
	}
	public void setRec_tax(BigDecimal rec_tax) {
		this.rec_tax = rec_tax;
	}
	public BigDecimal getRev_m_t() {
		return rev_m_t;
	}
	public void setRev_m_t(BigDecimal rev_m_t) {
		this.rev_m_t = rev_m_t;
	}
	public BigDecimal getGovt_grant() {
		return govt_grant;
	}
	public void setGovt_grant(BigDecimal govt_grant) {
		this.govt_grant = govt_grant;
	}
	public BigDecimal getLoan() {
		return loan;
	}
	public void setLoan(BigDecimal loan) {
		this.loan = loan;
	}
	public BigDecimal getAdvance() {
		return advance;
	}
	public void setAdvance(BigDecimal advance) {
		this.advance = advance;
	}
	public String getOth_sour() {
		return oth_sour;
	}
	public void setOth_sour(String oth_sour) {
		this.oth_sour = oth_sour;
	}
	public BigDecimal getTot_rec() {
		return tot_rec;
	}
	public void setTot_rec(BigDecimal tot_rec) {
		this.tot_rec = tot_rec;
	}
	public BigDecimal getGen_adm() {
		return gen_adm;
	}
	public void setGen_adm(BigDecimal gen_adm) {
		this.gen_adm = gen_adm;
	}
	public BigDecimal getPub_saf() {
		return pub_saf;
	}
	public void setPub_saf(BigDecimal pub_saf) {
		this.pub_saf = pub_saf;
	}
	public BigDecimal getPub_h_c() {
		return pub_h_c;
	}
	public void setPub_h_c(BigDecimal pub_h_c) {
		this.pub_h_c = pub_h_c;
	}
	public BigDecimal getPub_works() {
		return pub_works;
	}
	public void setPub_works(BigDecimal pub_works) {
		this.pub_works = pub_works;
	}
	public BigDecimal getPub_inst() {
		return pub_inst;
	}
	public void setPub_inst(BigDecimal pub_inst) {
		this.pub_inst = pub_inst;
	}
	public String getOth_exp() {
		return oth_exp;
	}
	public void setOth_exp(String oth_exp) {
		this.oth_exp = oth_exp;
	}
	public BigDecimal getTot_exp() {
		return tot_exp;
	}
	public void setTot_exp(BigDecimal tot_exp) {
		this.tot_exp = tot_exp;
	}
	public BigDecimal getK_road() {
		return k_road;
	}
	public void setK_road(BigDecimal k_road) {
		this.k_road = k_road;
	}
	public BigDecimal getP_road() {
		return p_road;
	}
	public void setP_road(BigDecimal p_road) {
		this.p_road = p_road;
	}
	public String getSewer() {
		return sewer;
	}
	public void setSewer(String sewer) {
		this.sewer = sewer;
	}
	public double getWater_bor() {
		return water_bor;
	}
	public void setWater_bor(double water_bor) {
		this.water_bor = water_bor;
	}
	public double getService() {
		return service;
	}
	public void setService(double service) {
		this.service = service;
	}
	public double getLat_oth() {
		return lat_oth;
	}
	public void setLat_oth(double lat_oth) {
		this.lat_oth = lat_oth;
	}
	public String getMeth_d_n_s() {
		return meth_d_n_s;
	}
	public void setMeth_d_n_s(String meth_d_n_s) {
		this.meth_d_n_s = meth_d_n_s;
	}
	public String getSou_of_sup() {
		return sou_of_sup;
	}
	public void setSou_of_sup(String sou_of_sup) {
		this.sou_of_sup = sou_of_sup;
	}
	public String getSys_stor() {
		return sys_stor;
	}
	public void setSys_stor(String sys_stor) {
		this.sys_stor = sys_stor;
	}
	public String getStor_cap() {
		return stor_cap;
	}
	public void setStor_cap(String stor_cap) {
		this.stor_cap = stor_cap;
	}
	public String getFire_f_ser() {
		return fire_f_ser;
	}
	public void setFire_f_ser(String fire_f_ser) {
		this.fire_f_ser = fire_f_ser;
	}
	public String getF_f_name() {
		return f_f_name;
	}
	public void setF_f_name(String f_f_name) {
		this.f_f_name = f_f_name;
	}
	public BigDecimal getDist_f_ser() {
		return dist_f_ser;
	}
	public void setDist_f_ser(BigDecimal dist_f_ser) {
		this.dist_f_ser = dist_f_ser;
	}
	public double getEl_dom() {
		return el_dom;
	}
	public void setEl_dom(double el_dom) {
		this.el_dom = el_dom;
	}
	public double getEl_ind() {
		return el_ind;
	}
	public void setEl_ind(double el_ind) {
		this.el_ind = el_ind;
	}
	public double getEl_com() {
		return el_com;
	}
	public void setEl_com(double el_com) {
		this.el_com = el_com;
	}
	public double getEl_road_l() {
		return el_road_l;
	}
	public void setEl_road_l(double el_road_l) {
		this.el_road_l = el_road_l;
	}
	public double getEl_others() {
		return el_others;
	}
	public void setEl_others(double el_others) {
		this.el_others = el_others;
	}
	public double getHospital() {
		return hospital;
	}
	public void setHospital(double hospital) {
		this.hospital = hospital;
	}
	public double getHos_beds() {
		return hos_beds;
	}
	public void setHos_beds(double hos_beds) {
		this.hos_beds = hos_beds;
	}
	public String getHt_name() {
		return ht_name;
	}
	public void setHt_name(String ht_name) {
		this.ht_name = ht_name;
	}
	public double getDisp() {
		return disp;
	}
	public void setDisp(double disp) {
		this.disp = disp;
	}
	public double getDisp_beds() {
		return disp_beds;
	}
	public void setDisp_beds(double disp_beds) {
		this.disp_beds = disp_beds;
	}
	public String getDt_name() {
		return dt_name;
	}
	public void setDt_name(String dt_name) {
		this.dt_name = dt_name;
	}
	public double getHealth_ctr() {
		return health_ctr;
	}
	public void setHealth_ctr(double health_ctr) {
		this.health_ctr = health_ctr;
	}
	public double getHc_beds() {
		return hc_beds;
	}
	public void setHc_beds(double hc_beds) {
		this.hc_beds = hc_beds;
	}
	public String getHct_name() {
		return hct_name;
	}
	public void setHct_name(String hct_name) {
		this.hct_name = hct_name;
	}
	public double getF_w_ctr() {
		return f_w_ctr;
	}
	public void setF_w_ctr(double f_w_ctr) {
		this.f_w_ctr = f_w_ctr;
	}
	public double getFwc_beds() {
		return fwc_beds;
	}
	public void setFwc_beds(double fwc_beds) {
		this.fwc_beds = fwc_beds;
	}
	public String getFwct_name() {
		return fwct_name;
	}
	public void setFwct_name(String fwct_name) {
		this.fwct_name = fwct_name;
	}
	public double getTb_clinic() {
		return tb_clinic;
	}
	public void setTb_clinic(double tb_clinic) {
		this.tb_clinic = tb_clinic;
	}
	public double getTbc_beds() {
		return tbc_beds;
	}
	public void setTbc_beds(double tbc_beds) {
		this.tbc_beds = tbc_beds;
	}
	public String getTbc_name() {
		return tbc_name;
	}
	public void setTbc_name(String tbc_name) {
		this.tbc_name = tbc_name;
	}
	public double getNur_homes() {
		return nur_homes;
	}
	public void setNur_homes(double nur_homes) {
		this.nur_homes = nur_homes;
	}
	public double getNh_beds() {
		return nh_beds;
	}
	public void setNh_beds(double nh_beds) {
		this.nh_beds = nh_beds;
	}
	public String getNht_name() {
		return nht_name;
	}
	public void setNht_name(String nht_name) {
		this.nht_name = nht_name;
	}
	public double getMed_oth() {
		return med_oth;
	}
	public void setMed_oth(double med_oth) {
		this.med_oth = med_oth;
	}
	public double getOthmi_beds() {
		return othmi_beds;
	}
	public void setOthmi_beds(double othmi_beds) {
		this.othmi_beds = othmi_beds;
	}
	public double getArts() {
		return arts;
	}
	public void setArts(double arts) {
		this.arts = arts;
	}
	public double getScience() {
		return science;
	}
	public void setScience(double science) {
		this.science = science;
	}
	public double getCommerce() {
		return commerce;
	}
	public void setCommerce(double commerce) {
		this.commerce = commerce;
	}
	public double getArts_sci() {
		return arts_sci;
	}
	public void setArts_sci(double arts_sci) {
		this.arts_sci = arts_sci;
	}
	public double getArts_comm() {
		return arts_comm;
	}
	public void setArts_comm(double arts_comm) {
		this.arts_comm = arts_comm;
	}
	public double getArt_sc_co() {
		return art_sc_co;
	}
	public void setArt_sc_co(double art_sc_co) {
		this.art_sc_co = art_sc_co;
	}
	public double getLaw() {
		return law;
	}
	public void setLaw(double law) {
		this.law = law;
	}
	public double getUniv() {
		return univ;
	}
	public void setUniv(double univ) {
		this.univ = univ;
	}
	public double getOth_col() {
		return oth_col;
	}
	public void setOth_col(double oth_col) {
		this.oth_col = oth_col;
	}
	public String getAs_near_km() {
		return as_near_km;
	}
	public void setAs_near_km(String as_near_km) {
		this.as_near_km = as_near_km;
	}
	public double getMedical() {
		return medical;
	}
	public void setMedical(double medical) {
		this.medical = medical;
	}
	public String getMd_near_km() {
		return md_near_km;
	}
	public void setMd_near_km(String md_near_km) {
		this.md_near_km = md_near_km;
	}
	public double getEngg() {
		return engg;
	}
	public void setEngg(double engg) {
		this.engg = engg;
	}
	public String getEn_near_km() {
		return en_near_km;
	}
	public void setEn_near_km(String en_near_km) {
		this.en_near_km = en_near_km;
	}
	public double getPolytech() {
		return polytech;
	}
	public void setPolytech(double polytech) {
		this.polytech = polytech;
	}
	public String getPoly_near() {
		return poly_near;
	}
	public void setPoly_near(String poly_near) {
		this.poly_near = poly_near;
	}
	public double getVt_inst() {
		return vt_inst;
	}
	public void setVt_inst(double vt_inst) {
		this.vt_inst = vt_inst;
	}
	public double getSh_inst() {
		return sh_inst;
	}
	public void setSh_inst(double sh_inst) {
		this.sh_inst = sh_inst;
	}
	public double getType_inst() {
		return type_inst;
	}
	public void setType_inst(double type_inst) {
		this.type_inst = type_inst;
	}
	public double getSh_type_in() {
		return sh_type_in;
	}
	public void setSh_type_in(double sh_type_in) {
		this.sh_type_in = sh_type_in;
	}
	public double getOth_inst_s() {
		return oth_inst_s;
	}
	public void setOth_inst_s(double oth_inst_s) {
		this.oth_inst_s = oth_inst_s;
	}
	public BigDecimal getS_sec() {
		return s_sec;
	}
	public void setS_sec(BigDecimal s_sec) {
		this.s_sec = s_sec;
	}
	public String getSs_near() {
		return ss_near;
	}
	public void setSs_near(String ss_near) {
		this.ss_near = ss_near;
	}
	public double getSecondary() {
		return secondary;
	}
	public void setSecondary(double secondary) {
		this.secondary = secondary;
	}
	public String getSec_near() {
		return sec_near;
	}
	public void setSec_near(String sec_near) {
		this.sec_near = sec_near;
	}
	public double getMiddle() {
		return middle;
	}
	public void setMiddle(double middle) {
		this.middle = middle;
	}
	public String getMid_near() {
		return mid_near;
	}
	public void setMid_near(String mid_near) {
		this.mid_near = mid_near;
	}
	public double get__primary() {
		return __primary;
	}
	public void set__primary(double __primary) {
		this.__primary = __primary;
	}
	public String getPrim_near() {
		return prim_near;
	}
	public void setPrim_near(String prim_near) {
		this.prim_near = prim_near;
	}
	public double getAdult_cnt() {
		return adult_cnt;
	}
	public void setAdult_cnt(double adult_cnt) {
		this.adult_cnt = adult_cnt;
	}
	public String getAlc_near() {
		return alc_near;
	}
	public void setAlc_near(String alc_near) {
		this.alc_near = alc_near;
	}
	public double getOth_cntr() {
		return oth_cntr;
	}
	public void setOth_cntr(double oth_cntr) {
		this.oth_cntr = oth_cntr;
	}
	public double getN_hostels() {
		return n_hostels;
	}
	public void setN_hostels(double n_hostels) {
		this.n_hostels = n_hostels;
	}
	public double getN_seats() {
		return n_seats;
	}
	public void setN_seats(double n_seats) {
		this.n_seats = n_seats;
	}
	public String getWwh_dist() {
		return wwh_dist;
	}
	public void setWwh_dist(String wwh_dist) {
		this.wwh_dist = wwh_dist;
	}
	public double getRc_fac() {
		return rc_fac;
	}
	public void setRc_fac(double rc_fac) {
		this.rc_fac = rc_fac;
	}
	public double getStadium() {
		return stadium;
	}
	public void setStadium(double stadium) {
		this.stadium = stadium;
	}
	public double getCinema() {
		return cinema;
	}
	public void setCinema(double cinema) {
		this.cinema = cinema;
	}
	public double getAuditorium() {
		return auditorium;
	}
	public void setAuditorium(double auditorium) {
		this.auditorium = auditorium;
	}
	public double getP_lib() {
		return p_lib;
	}
	public void setP_lib(double p_lib) {
		this.p_lib = p_lib;
	}
	public double getR_rooms() {
		return r_rooms;
	}
	public void setR_rooms(double r_rooms) {
		this.r_rooms = r_rooms;
	}
	public String getImp_com1() {
		return imp_com1;
	}
	public void setImp_com1(String imp_com1) {
		this.imp_com1 = imp_com1;
	}
	public String getImp_com2() {
		return imp_com2;
	}
	public void setImp_com2(String imp_com2) {
		this.imp_com2 = imp_com2;
	}
	public String getImp_com3() {
		return imp_com3;
	}
	public void setImp_com3(String imp_com3) {
		this.imp_com3 = imp_com3;
	}
	public String getExp_comm1() {
		return exp_comm1;
	}
	public void setExp_comm1(String exp_comm1) {
		this.exp_comm1 = exp_comm1;
	}
	public String getExp_comm2() {
		return exp_comm2;
	}
	public void setExp_comm2(String exp_comm2) {
		this.exp_comm2 = exp_comm2;
	}
	public String getExp_comm3() {
		return exp_comm3;
	}
	public void setExp_comm3(String exp_comm3) {
		this.exp_comm3 = exp_comm3;
	}
	public String getMan_comm1() {
		return man_comm1;
	}
	public void setMan_comm1(String man_comm1) {
		this.man_comm1 = man_comm1;
	}
	public String getMan_comm2() {
		return man_comm2;
	}
	public void setMan_comm2(String man_comm2) {
		this.man_comm2 = man_comm2;
	}
	public String getMan_comm3() {
		return man_comm3;
	}
	public void setMan_comm3(String man_comm3) {
		this.man_comm3 = man_comm3;
	}
	public double getBanks() {
		return banks;
	}
	public void setBanks(double banks) {
		this.banks = banks;
	}
	public double getA_c_soc() {
		return a_c_soc;
	}
	public void setA_c_soc(double a_c_soc) {
		this.a_c_soc = a_c_soc;
	}
	public double getN_a_c_soc() {
		return n_a_c_soc;
	}
	public void setN_a_c_soc(double n_a_c_soc) {
		this.n_a_c_soc = n_a_c_soc;
	}
	public String getDistcode() {
		return distcode;
	}
	public void setDistcode(String distcode) {
		this.distcode = distcode;
	}
	public String getTowncode() {
		return towncode;
	}
	public void setTowncode(String towncode) {
		this.towncode = towncode;
	}
	public Integer getStateid() {
		return stateid;
	}
	public void setStateid(Integer stateid) {
		this.stateid = stateid;
	}
	public Integer getDistid() {
		return distid;
	}
	public void setDistid(Integer distid) {
		this.distid = distid;
	}
	public Integer getTehsilid() {
		return tehsilid;
	}
	public void setTehsilid(Integer tehsilid) {
		this.tehsilid = tehsilid;
	}
}
